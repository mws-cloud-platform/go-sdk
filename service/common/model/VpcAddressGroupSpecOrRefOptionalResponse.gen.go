// Code generated by mwsapigen, DO NOT EDIT.

package model

import (
	"context"

	"github.com/go-faster/jx"

	"go.mws.cloud/go-sdk/internal/conv"
	reserrors "go.mws.cloud/go-sdk/internal/resources/errors"
	"go.mws.cloud/go-sdk/pkg/optional"
	"go.mws.cloud/go-sdk/service/resources/references/vpc"
)

// Описывает ссылку на группу адресов или спецификацию группы адресов.
// Real OAPI model name: VpcAddressGroupSpecOrRef
type VpcAddressGroupSpecOrRefOptionalResponse struct {
	// Ссылка на группу адресов.
	Ref optional.OptionalNil[vpc.AddressGroupRef] `json:"ref,omitempty" yaml:"ref,omitempty"`
	// Спецификация группы адресов.
	Spec optional.OptionalNil[VpcAddressGroupSpecOptionalResponse] `json:"spec,omitempty" yaml:"spec,omitempty"`
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) GetRef() *vpc.AddressGroupRef {
	if m != nil && m.Ref.IsSet() && !m.Ref.IsNull() {
		return &m.Ref.Value
	}
	return nil
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) GetRefOr(val vpc.AddressGroupRef) vpc.AddressGroupRef {
	if m != nil && m.Ref.IsSet() && !m.Ref.IsNull() {
		return m.Ref.Value
	}
	return val
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) GetSpec() *VpcAddressGroupSpecOptionalResponse {
	if m != nil && m.Spec.IsSet() && !m.Spec.IsNull() {
		return &m.Spec.Value
	}
	return nil
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) GetSpecOr(val VpcAddressGroupSpecOptionalResponse) VpcAddressGroupSpecOptionalResponse {
	if m != nil && m.Spec.IsSet() && !m.Spec.IsNull() {
		return m.Spec.Value
	}
	return val
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) Clone() *VpcAddressGroupSpecOrRefOptionalResponse {
	if m == nil {
		return nil
	}

	clone := *m
	if clone.Ref.IsSet() {
		clone.Ref.Value = *m.Ref.Value.Clone()
	}
	if clone.Spec.IsSet() {
		clone.Spec.Value = *m.Spec.Value.Clone()
	}
	return &clone
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) Parse(ctx context.Context) error {
	if m == nil {
		return nil
	}

	if m.Ref.IsSet() && !m.Ref.IsNull() {
		if err := m.Ref.Value.Parse(ctx); err != nil {
			return reserrors.NewPathAccumulatorError("Ref", err)
		}
	}

	if m.Spec.IsSet() && !m.Spec.IsNull() {
		if err := m.Spec.Value.Parse(ctx); err != nil {
			return reserrors.NewPathAccumulatorError("Spec", err)
		}
	}

	return nil
}

// JSON methods

func (m VpcAddressGroupSpecOrRefOptionalResponse) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	if err := m.Encode(&e); err != nil {
		return nil, err
	}
	return e.Bytes(), nil
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) Encode(e *jx.Encoder) error {
	if m == nil {
		e.Null()
		return nil
	}
	e.ObjStart()
	if err := m.encodeFields(e); err != nil {
		return err
	}
	e.ObjEnd()
	return nil
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) encodeFields(e *jx.Encoder) error {
	if m.Ref.IsSet() {
		e.FieldStart("ref")
		if m.Ref.IsNull() {
			e.Null()
		} else {
			if err := m.Ref.Value.Encode(e); err != nil {
				return err
			}
		}
	}

	if m.Spec.IsSet() {
		e.FieldStart("spec")
		if m.Spec.IsNull() {
			e.Null()
		} else {
			if err := m.Spec.Value.Encode(e); err != nil {
				return err
			}
		}
	}
	return nil
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) UnmarshalJSON(b []byte) error {
	return m.Decode(jx.DecodeBytes(b))
}

func (m *VpcAddressGroupSpecOrRefOptionalResponse) Decode(d *jx.Decoder) error {
	if m == nil {
		return conv.NewDecodeToNilError("VpcAddressGroupSpecOrRefOptionalResponse")
	}

	return d.ObjBytes(reserrors.PathAccumulatorErrorObjBytesFuncWrap(func(d *jx.Decoder, k []byte) error {
		switch string(k) {
		case "ref":
			if d.Next() == jx.Null {
				m.Ref.SetToNull()
				return d.Null()
			}

			var v vpc.AddressGroupRef
			if err := v.Decode(d); err != nil {
				return err
			}

			m.Ref.SetTo(v)
			return nil
		case "spec":
			if d.Next() == jx.Null {
				m.Spec.SetToNull()
				return d.Null()
			}

			var v VpcAddressGroupSpecOptionalResponse
			if err := v.Decode(d); err != nil {
				return err
			}

			m.Spec.SetTo(v)
			return nil
		default:
			return d.Skip()
		}
	}))
}
