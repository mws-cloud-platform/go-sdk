...
func TestEnableServiceSugared_BatchEnableServicesV2(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	mockClient := clientmocks.NewMockEnableService(ctrl)
	mockClient.EXPECT().BatchEnableServicesV2(gomock.Any(), gomock.Any()).Times(1).Return(nil, mwserrors.NewDecodeBodyError("", nil))

	sugared := client.NewEnableServiceSugared(mockClient)
...
func TestEnableServiceSugared_ListEnabledServices(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestEnableServiceSugared_EnableService(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
