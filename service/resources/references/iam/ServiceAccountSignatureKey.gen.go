// Code generated by mwsapigen, DO NOT EDIT.

package iam

import (
	"context"
	"fmt"

	"github.com/go-faster/jx"

	"go.mws.cloud/go-sdk/internal/conv"
	"go.mws.cloud/go-sdk/internal/decode"
	reserrors "go.mws.cloud/go-sdk/internal/resources/errors"
	resparsers "go.mws.cloud/go-sdk/internal/resources/parsers"
	valuesctx "go.mws.cloud/go-sdk/pkg/context/values"
	resifaces "go.mws.cloud/go-sdk/pkg/resources/interfaces"
	resmodels "go.mws.cloud/go-sdk/pkg/resources/models"
)

var (
	ServiceAccountSignatureKeyRefTemplate = resparsers.Template{
		{
			Value:       "keyName",
			IsConstant:  false,
			SearchAfter: false,
		},
		{
			Value:       "signatureKeys",
			IsConstant:  true,
			SearchAfter: false,
		},
		{
			Value:       "serviceAccountId",
			IsConstant:  false,
			SearchAfter: true,
		},
		{
			Value:       "serviceAccounts",
			IsConstant:  true,
			SearchAfter: false,
		},
		{
			Value:       "project",
			IsConstant:  false,
			SearchAfter: true,
		},
		{
			Value:       "projects",
			IsConstant:  true,
			SearchAfter: false,
		},
		{
			Value:       "iam",
			IsConstant:  true,
			SearchAfter: false,
		},
	}
)

func NewServiceAccountSignatureKeyID(project, serviceAccountId, keyName string) (ServiceAccountSignatureKeyID, error) {
	if keyName == "" {
		return ServiceAccountSignatureKeyID{}, reserrors.NewFieldIsEmptyError("keyName")
	}
	if serviceAccountId == "" {
		return ServiceAccountSignatureKeyID{}, reserrors.NewFieldIsEmptyError("serviceAccountId")
	}
	if project == "" {
		return ServiceAccountSignatureKeyID{}, reserrors.NewFieldIsEmptyError("project")
	}
	m := ServiceAccountSignatureKeyID{
		keyName:          keyName,
		serviceAccountId: serviceAccountId,
		project:          project,
	}
	m.path = m.ID()
	return m, nil
}

func NewMustServiceAccountSignatureKeyID(project, serviceAccountId, keyName string) ServiceAccountSignatureKeyID {
	m, err := NewServiceAccountSignatureKeyID(project, serviceAccountId, keyName)
	if err != nil {
		panic(err)
	}
	return m
}

func ParseServiceAccountSignatureKeyID(path string) (ServiceAccountSignatureKeyID, error) {
	m := ServiceAccountSignatureKeyID{
		path: path,
	}
	if err := m.parse(); err != nil {
		return ServiceAccountSignatureKeyID{}, err
	}
	return m, nil
}

func NewServiceAccountSignatureKeyIDFromAnyID(resource resmodels.AnyResourceID) (ServiceAccountSignatureKeyID, error) {
	return ParseServiceAccountSignatureKeyID(resource.ID())
}

type ServiceAccountSignatureKeyID struct {
	keyName          string
	serviceAccountId string
	project          string
	path             string
}

func (m *ServiceAccountSignatureKeyID) ResourceName() resifaces.ResourceName {
	if m == nil {
		return ""
	}
	return resifaces.ResourceName(m.keyName)
}

func (m *ServiceAccountSignatureKeyID) GetKeyName() string {
	if m == nil {
		return ""
	}
	return m.keyName
}

func (m *ServiceAccountSignatureKeyID) GetServiceAccountId() string {
	if m == nil {
		return ""
	}
	return m.serviceAccountId
}

func (m *ServiceAccountSignatureKeyID) GetProject() string {
	if m == nil {
		return ""
	}
	return m.project
}

func (m *ServiceAccountSignatureKeyID) ServiceSlug() string {
	return "iam"
}

func (m *ServiceAccountSignatureKeyID) ID() string {
	if m == nil || m.keyName == "" || m.serviceAccountId == "" || m.project == "" {
		return ""
	}

	return m.ServiceSlug() + "/projects/" + m.project + "/serviceAccounts/" + m.serviceAccountId + "/signatureKeys/" + m.keyName
}

func (m *ServiceAccountSignatureKeyID) String() string {
	return m.ID()
}

func (m *ServiceAccountSignatureKeyID) Clone() *ServiceAccountSignatureKeyID {
	if m == nil {
		return nil
	}
	clone := *m
	return &clone
}

func (m *ServiceAccountSignatureKeyID) AsRef() *ServiceAccountSignatureKeyRef {
	if m == nil {
		return nil
	}
	return &ServiceAccountSignatureKeyRef{
		id: *m,
	}
}

func (m *ServiceAccountSignatureKeyID) Equal(other *ServiceAccountSignatureKeyID) bool {
	if m == other {
		return true
	}
	if m == nil || other == nil {
		return false
	}
	return m.keyName == other.keyName && m.serviceAccountId == other.serviceAccountId && m.project == other.project
}

func (m ServiceAccountSignatureKeyID) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	if err := m.Encode(&e); err != nil {
		return nil, err
	}
	return e.Bytes(), nil
}

func (m *ServiceAccountSignatureKeyID) Encode(e *jx.Encoder) error {
	if m == nil {
		e.Null()
		return nil
	}
	result := m.ID()
	if result == "" {
		return fmt.Errorf("encode id: %w", reserrors.ErrIDIsEmpty)
	}
	e.Str(result)
	return nil
}

func (m *ServiceAccountSignatureKeyID) UnmarshalJSON(b []byte) error {
	return m.Decode(jx.DecodeBytes(b))
}

func (m *ServiceAccountSignatureKeyID) Decode(d *jx.Decoder) error {
	if m == nil {
		return conv.NewDecodeToNilError("ServiceAccountSignatureKeyID")
	}

	v, err := decode.Str(d)
	if err != nil {
		return err
	}

	m.path = v
	return m.parse()
}

// Deprecated: Parse method is no longer required.
// Internal fields are populated automatically during decoding.
// This method will be removed in the next SDK release.
func (m *ServiceAccountSignatureKeyID) Parse(ctx context.Context) error {
	return nil
}

func (m *ServiceAccountSignatureKeyID) parse() error {
	if m == nil {
		return nil
	}

	if m.path == "" {
		return reserrors.NewParseIDError("", reserrors.ErrPathIsEmpty)
	}

	result, err := resparsers.Reference(context.Background(), m.path, ServiceAccountSignatureKeyRefTemplate.AsID())
	if err != nil {
		return reserrors.NewParseIDError(m.path, err)
	}

	m.keyName = result["keyName"]
	m.serviceAccountId = result["serviceAccountId"]
	m.project = result["project"]

	return nil
}

func NewServiceAccountSignatureKeyRef(project, serviceAccountId, keyName string) (ServiceAccountSignatureKeyRef, error) {
	if keyName == "" {
		return ServiceAccountSignatureKeyRef{}, reserrors.NewFieldIsEmptyError("keyName")
	}
	if serviceAccountId == "" {
		return ServiceAccountSignatureKeyRef{}, reserrors.NewFieldIsEmptyError("serviceAccountId")
	}
	if project == "" {
		return ServiceAccountSignatureKeyRef{}, reserrors.NewFieldIsEmptyError("project")
	}
	m := ServiceAccountSignatureKeyRef{
		id: ServiceAccountSignatureKeyID{
			keyName:          keyName,
			serviceAccountId: serviceAccountId,
			project:          project,
		},
	}
	m.id.path = m.absolutePath()
	return m, nil
}

func NewMustServiceAccountSignatureKeyRef(project, serviceAccountId, keyName string) ServiceAccountSignatureKeyRef {
	m, err := NewServiceAccountSignatureKeyRef(project, serviceAccountId, keyName)
	if err != nil {
		panic(err)
	}
	return m
}

func ParseServiceAccountSignatureKeyRef(ctx context.Context, path string) (ServiceAccountSignatureKeyRef, error) {
	m := ServiceAccountSignatureKeyRef{
		id: ServiceAccountSignatureKeyID{
			path: path,
		},
	}
	if err := m.Parse(ctx); err != nil {
		return ServiceAccountSignatureKeyRef{}, err
	}
	return m, nil
}

func NewServiceAccountSignatureKeyRefFromAnyRef(ref resmodels.AnyResourceRef) (ServiceAccountSignatureKeyRef, error) {
	ctx := valuesctx.WithValuesStore(context.Background(), ref.GetPathValues())
	return ParseServiceAccountSignatureKeyRef(ctx, ref.Path())
}

type ServiceAccountSignatureKeyRef struct {
	id ServiceAccountSignatureKeyID
}

func (m *ServiceAccountSignatureKeyRef) ResourceName() resifaces.ResourceName {
	if m == nil {
		return ""
	}
	return m.id.ResourceName()
}

func (m *ServiceAccountSignatureKeyRef) GetKeyName() string {
	if m == nil {
		return ""
	}
	return m.id.GetKeyName()
}

func (m *ServiceAccountSignatureKeyRef) GetServiceAccountId() string {
	if m == nil {
		return ""
	}
	return m.id.GetServiceAccountId()
}

func (m *ServiceAccountSignatureKeyRef) GetProject() string {
	if m == nil {
		return ""
	}
	return m.id.GetProject()
}

func (m *ServiceAccountSignatureKeyRef) ServiceSlug() string {
	return "iam"
}

func (m *ServiceAccountSignatureKeyRef) IDPath() string {
	if m == nil {
		return ""
	}
	return m.id.ID()
}

func (m *ServiceAccountSignatureKeyRef) Path() string {
	if m == nil {
		return ""
	}
	if m.id.path == "" {
		return m.absolutePath()
	}
	return m.id.path
}

func (m *ServiceAccountSignatureKeyRef) String() string {
	return m.IDPath()
}

func (m *ServiceAccountSignatureKeyRef) Parse(ctx context.Context) error {
	return m.parse(ctx, false)
}

func (m *ServiceAccountSignatureKeyRef) Clone() *ServiceAccountSignatureKeyRef {
	if m == nil {
		return nil
	}
	clone := *m
	return &clone
}

func (m *ServiceAccountSignatureKeyRef) Equal(other *ServiceAccountSignatureKeyRef) bool {
	if m == other {
		return true
	}
	if m == nil || other == nil {
		return false
	}
	return m.id.keyName == other.id.keyName && m.id.serviceAccountId == other.id.serviceAccountId && m.id.project == other.id.project
}

func (m ServiceAccountSignatureKeyRef) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	if err := m.Encode(&e); err != nil {
		return nil, err
	}
	return e.Bytes(), nil
}

func (m *ServiceAccountSignatureKeyRef) Encode(e *jx.Encoder) error {
	if m == nil {
		e.Null()
		return nil
	}
	result := m.Path()
	if result == "" {
		return fmt.Errorf("encode reference: %w", reserrors.ErrPathIsEmpty)
	}
	e.Str(result)
	return nil
}

func (m *ServiceAccountSignatureKeyRef) UnmarshalJSON(b []byte) error {
	return m.Decode(jx.DecodeBytes(b))
}

func (m *ServiceAccountSignatureKeyRef) Decode(d *jx.Decoder) error {
	if m == nil {
		return conv.NewDecodeToNilError("ServiceAccountSignatureKeyRef")
	}

	v, err := decode.Str(d)
	if err != nil {
		return err
	}

	m.id.path = v
	return m.parse(context.Background(), true)
}

func (m *ServiceAccountSignatureKeyRef) parse(ctx context.Context, allowPartial bool) error {
	if m == nil || m.isParsed() {
		return nil
	}

	if m.id.path == "" {
		return reserrors.NewParseReferenceError("", reserrors.ErrPathIsEmpty)
	}

	var options []resparsers.Option
	if allowPartial {
		options = append(options, resparsers.AllowPartial())
	}

	result, err := resparsers.Reference(ctx, m.id.path, ServiceAccountSignatureKeyRefTemplate, options...)
	if err != nil {
		return reserrors.NewParseReferenceError(m.id.path, err)
	}

	m.id.keyName = result["keyName"]
	m.id.serviceAccountId = result["serviceAccountId"]
	m.id.project = result["project"]

	return nil
}

func (m *ServiceAccountSignatureKeyRef) isParsed() bool {
	return m != nil && m.id.keyName != "" && m.id.serviceAccountId != "" && m.id.project != ""
}

func (m *ServiceAccountSignatureKeyRef) absolutePath() string {
	if m == nil || m.id.keyName == "" || m.id.serviceAccountId == "" || m.id.project == "" {
		return ""
	}

	return "projects/" + m.id.project + "/serviceAccounts/" + m.id.serviceAccountId + "/signatureKeys/" + m.id.keyName
}
