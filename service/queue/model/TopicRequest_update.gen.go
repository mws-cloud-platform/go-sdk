// Code generated by mwsapigen, DO NOT EDIT.

package model

import (
	"go.mws.cloud/util-toolset/pkg/utils/ptr"

	"go.mws.cloud/go-sdk/pkg/optional"
	commonmodel "go.mws.cloud/go-sdk/service/common/model"
)

type UpdateTopicRequest struct {
	// Набор общих для всех пользовательских объектов атрибутов. Может быть расширен атрибутами, специфичными для контейнеров.
	Metadata optional.OptionalNil[commonmodel.UpdateCommonTypedResourceMetadataRequest] `json:"metadata" yaml:"metadata"`
	// Спецификация топика
	Spec optional.Optional[UpdateTopicSpecRequest] `json:"spec" yaml:"spec"`
}

func (m *TopicRequest) AsUpdateModel() UpdateTopicRequest {
	var u UpdateTopicRequest
	if m.Metadata != nil {
		u.Metadata = optional.NewOptionalNil(m.Metadata.AsUpdateModel())
	}
	u.Spec = optional.NewOptional(m.Spec.AsUpdateModel())
	return u
}

// Diff creates an object that can be used in Update methods. This object represents changes from src to the current state
func (m *TopicRequest) Diff(src *TopicRequest) UpdateTopicRequest {
	nilDiffers := src != nil && m == nil
	upd := UpdateTopicRequest{}
	if !nilDiffers {
		upd.Metadata = m.diffMetadata(src)
		upd.Spec = m.diffSpec(src)
	}
	return upd
}

func (m *TopicRequest) WithChanges(u UpdateTopicRequest) TopicRequest {
	var out TopicRequest
	if m != nil {
		out = *m
	}

	if u.Metadata.IsSet() {
		out.Metadata = ptr.Get(out.Metadata.WithChanges(u.Metadata.Value))
	} else if u.Metadata.IsNull() {
		out.Metadata = nil
	}
	if u.Spec.IsSet() {
		out.Spec = out.Spec.WithChanges(u.Spec.Value)
	}
	return out
}

// HasChanges returns true if any field has Set == true
func (m UpdateTopicRequest) HasChanges() bool {
	return m.Metadata.Set ||
		m.Spec.Set
}

func (m *TopicRequest) diffMetadata(src *TopicRequest) optional.OptionalNil[commonmodel.UpdateCommonTypedResourceMetadataRequest] {
	nilDiffers := src != nil && m == nil
	value := m.GetMetadata().Diff(src.GetMetadata())
	return optional.OptionalNil[commonmodel.UpdateCommonTypedResourceMetadataRequest]{
		Value: value,
		Set:   nilDiffers || value.HasChanges(),
		Null:  nilDiffers,
	}
}

func (m *TopicRequest) diffSpec(src *TopicRequest) optional.Optional[UpdateTopicSpecRequest] {
	from := src.GetSpec()
	to := m.GetSpec()
	value := to.Diff(&from)
	return optional.Optional[UpdateTopicSpecRequest]{
		Value: value,
		Set:   value.HasChanges(),
	}
}
