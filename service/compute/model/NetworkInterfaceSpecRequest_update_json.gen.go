// Code generated by mwsapigen, DO NOT EDIT.

package model

import (
	"github.com/go-faster/jx"

	"go.mws.cloud/go-sdk/internal/conv"
	"go.mws.cloud/go-sdk/internal/decode"
	reserrors "go.mws.cloud/go-sdk/internal/resources/errors"
)

func (m UpdateNetworkInterfaceSpecRequest) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	m.Encode(&e)
	return e.Bytes(), nil
}

func (m *UpdateNetworkInterfaceSpecRequest) Encode(e *jx.Encoder) {
	if m == nil {
		e.Null()
		return
	}
	e.ObjStart()
	m.encodeFields(e)
	e.ObjEnd()
}

func (m *UpdateNetworkInterfaceSpecRequest) encodeFields(e *jx.Encoder) {
	if m.Name.IsSet() {
		e.FieldStart("name")
		e.Str(m.Name.Value)
	}

	if m.Primary.IsSet() {
		e.FieldStart("primary")
		e.Bool(m.Primary.Value)
	}

	if m.Addresses.IsSet() {
		e.FieldStart("addresses")
		e.ArrStart()
		for _, elem := range m.Addresses.Value {
			elem.Encode(e)
		}
		e.ArrEnd()
	}
}

func (m *UpdateNetworkInterfaceSpecRequest) UnmarshalJSON(b []byte) error {
	return m.Decode(jx.DecodeBytes(b))
}

func (m *UpdateNetworkInterfaceSpecRequest) Decode(d *jx.Decoder) error {
	if m == nil {
		return conv.NewDecodeToNilError("UpdateNetworkInterfaceSpecRequest")
	}

	return d.ObjBytes(reserrors.PathAccumulatorErrorObjBytesFuncWrap(func(d *jx.Decoder, k []byte) error {
		switch string(k) {
		case "name":
			v, err := decode.Str(d)
			if err != nil {
				return err
			}

			m.Name.SetTo(v)
			return nil
		case "primary":
			v, err := decode.Bool(d)
			if err != nil {
				return err
			}

			m.Primary.SetTo(v)
			return nil
		case "addresses":
			c := make([]UpdateAddressSpecOrRefWithAttachmentsRequest, 0)
			if err := d.Arr(reserrors.PathAccumulatorErrorAsIndexArrFuncWrap(func(d *jx.Decoder) error {
				var v UpdateAddressSpecOrRefWithAttachmentsRequest
				if err := v.Decode(d); err != nil {
					return err
				}
				c = append(c, v)
				return nil
			})); err != nil {
				return err
			}

			m.Addresses.SetTo(c)
			return nil
		default:
			return d.Skip()
		}
	}))
}
