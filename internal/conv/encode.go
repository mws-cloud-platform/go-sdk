...
// Вероятно не все методы понадобятся. Пока что просто их скопировал из ogen
...
func StringToStringErr(v string) (string, error) { return v, nil }
func BytesToString(v []byte) string              { return string(v) }
func StringToStringSubset[T ~string](s string) T { return T(s) }

func TimeToString(v time.Time) string        { return v.Format(timeLayout) }
func TimeUTCToString(v time.Time) string     { return v.UTC().Format(timeLayout) }
func DateToString(v time.Time) string        { return v.Format(dateLayout) }
func DateUTCToString(v time.Time) string     { return v.UTC().Format(dateLayout) }
func DateTimeToString(v time.Time) string    { return v.Format(time.RFC3339) }
func DateTimeUTCToString(v time.Time) string { return v.UTC().Format(time.RFC3339) }
...
func BitrateToString(b bitrate.Bitrate, unit bitrate.Unit) string {
	return valueToString(b, unit, bitrate.BITS.String())
}

func ByteSizeToString(b bytesize.ByteSize, unit bytesize.Unit) string {
	return valueToString(b, unit, bytesize.B.String())
}

func FrequencyToString(f frequency.Frequency, unit frequency.Unit) string {
	return valueToString(f, unit, frequency.HZ.String())
}

func LargeNumberToString(l largenumber.LargeNumber, unit largenumber.Unit) string {
	return valueToString(l, unit, largenumber.EmptyUnit.String())
}

func ThroughputToString(t throughput.Throughput, unit throughput.Unit) string {
	return valueToString(t, unit, throughput.BPS.String())
}

func EncodeBase64(w io.Writer, payload []byte) error {
...
func valueToString[U unitKind[U]](value valueUnit, unit U, minUnitString string) string {
	if value.BigInt().Cmp(big.NewInt(0)) == 0 {
		// largenumber emptyUnitStr is empty
		if minUnitString != "" {
			return "0 " + minUnitString
		}
		return "0"
	}
	var (
	...
	for ; valid; unit, valid = unit.Decrease() {
		result = unitToString(value, unit)
		if divideUnit(value.BigInt(), unit.GetValue()).GreaterThanOrEqual(oneDecimal) {
			break
		}
	}
...
type valueUnit interface {
	BigInt() *big.Int
}

type unitKind[T any] interface {
	GetValue() *big.Int
	String() string
	Decrease() (T, bool)
}

func unitToString[T any](v valueUnit, u unitKind[T]) string {
	return divideUnit(v.BigInt(), u.GetValue()).String() + " " + u.String()
}

func divideUnit(base, unit *big.Int) decimal.Decimal {
	const roundPlaces = 2
	baseQuantityDecimal := decimal.NewFromBigInt(base, 0)
	unitDecimal := decimal.NewFromBigInt(unit, 0)
	if unitDecimal.IsZero() {
		unitDecimal = decimal.NewFromInt(1)
	}
	return baseQuantityDecimal.Div(unitDecimal).Round(roundPlaces)
}
